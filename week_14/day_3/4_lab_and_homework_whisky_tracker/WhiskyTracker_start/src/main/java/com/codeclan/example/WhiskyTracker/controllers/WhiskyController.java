...
public class WhiskyController {
    ...
    public ResponseEntity<List<Whisky>> getAllWhiskies(@RequestParam Optional<Integer> year, Optional<Integer> age,  Optional<String> name, Optional<String> region){
        if (year.isPresent()) {
            return new ResponseEntity<>(whiskyRepository.findByYear(year.get()), HttpStatus.OK);

        } else if (name.isPresent() && age.isPresent()) {
